...
const ReturnClosed = () => {
  ...
  const [paymentstatus, setPaymentStatus] = useState("Refund Status");
  ...
  const filterOneItems = ["Latest", "Latest Closed Date", "Oldest Closed Date"];
  ...
  const filteredOrders = useMemo(() => {
    ...
    /* 💳 RETURN TYPE (Return Type, Exchange, Return) */
    /* 💳 RETURN TYPE */
    ...
    /* 💳 PAYMENT STATUS */
    if (paymentstatus !== "Refund Status") {
      result = result.filter((item) => item.returnStatus === paymentstatus);
    }
    ...
    if (filterOne === "Latest") {
      result.sort(
        (a, b) =>
          new Date(b.trackingDetails.expectedDeliveryDate) -
          new Date(a.trackingDetails.expectedDeliveryDate),
      );
    }

    if (filterOne === "Latest Closed Date") {
      result.sort(
        (a, b) =>
          new Date(a.trackingDetails.expectedDeliveryDate) -
          new Date(b.trackingDetails.expectedDeliveryDate),
    ...
    if (filterOne === " Oldest Closed Date") {
      result.sort(
        (a, b) =>
          new Date(b.trackingDetails.expectedDeliveryDate) -
          new Date(a.trackingDetails.expectedDeliveryDate),
    ...
    return result;
  }, [returns, debouncedValue, returnType, paymentstatus, filterOne]);
            ...
            {paginatedOrders.map((order) => (
              <tr
                ...
                <td className={`px-4 py-3 font-medium text-xs  `}>
                  <span
                    className={`inline-flex items-center justify-center min-w-[110px] px-4 py-1.5 rounded-md font-medium text-center ${
                      order.returnStatus === "Refund Initiated"
                        ? "text-[#E91DD1] bg-[#FBDBF7]"
                        : order.returnStatus === "Refunded"
                          ? "text-[#8A38F5] bg-[#E6D3FF]"
                          : order.returnStatus === "Exchange Initiated"
                            ? "text-[#1C3753] bg-[#D5E5F5]"
                            : order.returnStatus === "Exchanged"
                              ? "text-[#008D94] bg-[#C7FCFF]"
                              : order.returnStatus === "Rejected"
                                ? "text-[#D53B35] bg-[#FFEAE9]"
                                : order.returnStatus === "Return to Origin"
                                  ? "text-[#686868] bg-[#DEDEDE]"
                                  : order.returnStatus === "No Refund"
                ...
                <td className="px-4 py-3">
                  ...
                  `}> */}
                  {order.trackingDetails.expectedDeliveryDate}
                  {/* </span> */}
                </td>
                ...
                <div className="flex items-center justify-center ">
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      className="p-2  text-[#1C3753] hover:underline">
                      View
                    </button>
                  </td>
                </div>
